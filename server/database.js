...
export function insertInitialData() {
  return new Promise((resolve, reject) => {
    ...
    const initialData = [
      { name: 'BaldBuck', platform: 'YouTube', description: 'Well, he really IS bald.' },
      { name: 'JordyBL', platform: 'TikTok', description: 'Sell me this pen!' },
      { name: 'FromMuskTillDawn', platform: 'Twitch', description: 'Who owns this place? 8)' },
    ];

    // SQL query to insert initial data
    const insertSql = `
      INSERT INTO streamers (name, platform, description)
      VALUES (?, ?, ?);
    `;
    ...
    const promises = initialData.map((data) => {
      return new Promise((resolve, reject) => {
        db.run(insertSql, [data.name, data.platform, data.description], function (error) {
          if (error) {
            reject(error);
          } else {
            resolve();
  ...
  return new Promise((resolve, reject) => {
    ...
    db.run(sql, [streamerData.name, streamerData.description, streamerData.platform], function (error) {
      ...
      if (error) {
        ...
        resolve({ id: this.lastID, ...streamerData });
      }
    });
  });
};
